...
In comparison, previous publications have either focused entirely on the physics performance \cite{Burleson:2882507, ctd23-gnn4itk}, 
or evaluated computing performance on an open dataset based on simplified geometry \cite{exatrkx}. 
...
It should be noted, however, that the computational performance of this technique undergoes rapid developments, 
and thus this chapter aims to provide a snapshot of the progress at the time of writing, rather than the finished product.
...
We detail in this section two optimizations which in combination significantly enhance the inference speed of the \ignn, 
starting with Automated Mixed Precision, followed by Just-In-Time compilation.
...
A number represented by binary form is characterized by three components, namely the sign, the exponent, and the mantissa, 
each of which is quantified by a number of bits, depending on the data format. 
...
However, during inference, such a wide range and high precision may not be necessary to achieve good accuracy, 
since no gradient calculation, which is prone to numerical explosion and vanishing, takes place. 
If the network output is stable under smaller bit widths, it is possible to decrease memory footprint, 
improve computational efficiency, and reduce power consumption by simply lowering the precision. 
...
Under the hood, \textsc{PyTorch} performs a series of complex transformations to analyze the computational graph\footnote{A model is essentially a computational graph, to be distinguished from the graph data on which it operates.} for efficient execution on both CPUs and GPUs. 
...
This weak scaling makes the computation less susceptible to unusually large events, as shown in figure \ref{subfig:gnn-time-node} above $\abs{V}=4.5\times 10^5$.
...
As of writing, a number of latency measurements are not available.
On graphs constructed by the Module Map MeanRMS approach, the latency of both graph construction and segmentation stages has not been thoroughly measured, and on those 
constructed by the Metric Learning approach, the latency of the graph segmentation stage is missing.
% However, we expect that the GPU 
We expect the latency to vary by a small margin among the three pipelines, thanks to the similar average size of the remaining graphs after applying a loose score cut in the Walkthrough algorithm.
The difference in graph construction latency of the two Module Map approaches remains to be determined.

Among all three variants of the GNN4ITk pipeline, the edge classification latency is measured with combined AMP and JIT optimizations.
Graph segmentation is the slowest stage, contributing nearly 60\% of the total run time, while graph construction and edge classification each account for 20\%.
...
A CUDA-based version of the Walkthrough algorith is under development, promising better latency in the future. 
...
\begin{table}[h!]
    ...
    \begin{tabular}{|l|c|c|c|} \hline
        ...
        Graph construction & & $41\pm 11$ & $932\pm 92$ \\
        Edge classification & $42\pm 9$ & $53\pm 12$ &  $47\pm10$  \\
        Graph segmentation & & $120\pm 93$ &  \\ \hline \hline
...
The metric learning step suffers from an intensive kNN search in a 12-dimensional space, and the filter step operates on large graphs of $\abs{V}\sim 6\times 10^6$ edges. 
...
As of writing the latency of the graph segmentation step has only been measured on graphs constructed by the Module Map MinMax method. 
