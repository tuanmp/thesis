...
The second stage of the GNN4ITk chain labels the graph connections, so that fake edges are removed and track candidates are built from exclusively true connections. 
...
Numerous GNN architectures have been proposed, from the simple Graph Convolutional Networks (GCNs) \cite{gcn}, 
which leverage spectral graph theory to define convolution-like operations on graphs, 
Graph Attention Networks (GATs) \cite{gat}, which introduce attention mechanisms for adaptive neighbourhood weighting, 
to GraphSAGE \cite{graphsage}, and Graph Isomorphism Networks (GINs) \cite{gin}, which improved expressiveness in distinguishing graph structures.

\section{The filter network}
\label{sect:chap-gnn-filter-network}

In the previous section, we have motivated and introduced GNNs as the deep learning method for non-Euclidean data represented as graphs. 
...
It is found in reference \cite{graphsage} that $K=2$ and sample sizes $S_1=25$, $S_2=10$ strike a good balance between memory and performance, which are used in the \textbf{Filter}.
...
The embeddings of two nodes $(v_i, v_j)$ connected by an edge $e_{ij} \in {E}$ are concatenated and fed to a decoder $\phi: \mathbb{R}^n\times \mathbb{R}^n\rightarrow (0,1)$ to obtain a score representing the probability of being a true edge.
...
The Filter network can be compared to the geometric cuts of the Module Map approach. 
Both serve to reduce the large number of edges present in graphs constructed by the k-Nearest-Neighbor or the Module Map 
by filtering out edges that do not meet some selection criteria.
The geometric cuts make reject the edge based entirely on local and hand-engineered edge features computed from the features of the connected nodes.
The Filter network, on the other hand, constructs a single edge score from the local node features and information from other nodes in the neighborhood, 
and makes a rejection decision based on it.
...
\begin{table}[h!]
    ...
    \begin{tabular}{l|c|c|c}
      ...
      \hline \hline
        Module Map {\textsc{MinMax}} & 99.69 & 1.08 & $1.22\times 10^6$ \\
        Module Map {\textsc{MeanRMS}} & 99.51  & 1.56 & $8.55\times 10^5$ \\
...
Table \ref{tab:graph-contruction-comparison} compares properties of graphs produced by Metric Learning and Filter network to those produced by the Module Map method. 
...
Indeed, very few other GNN architectures maintain edge-level intermediate vectors, since information exchange between nodes can be effectuated without an explicit edge state. 
...
Figures \ref{subfig:gnn-eff-eta-meanrms} and \ref{subfig:gnn-eff-pt-meanrms} describe the performance on graphs from the Module Map MeanRMS variant, whereas figures \ref{subfig:gnn-eff-eta-ml} and \ref{subfig:gnn-eff-pt-ml} those from the Metric Learning variant.
...
It is worth noting that the GNN4ITk pipeline has been developed over several iterations of simulation data, the most recent of which is described in reference \cite{ctd23-gnn4itk}, on a dataset of 1780 $t\bar{t}$-events.
...
The edge-based metrics are useful in evaluating machine learning models, and a simple score cut is typically sufficient in many applications. 
...
In addition, the resulting track candidiates must be fed to Athena \cite{atlas_collaboration_2021_4772550}, the main software analysis framework of ATLAS, 
