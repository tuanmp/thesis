...
proton-proton collision data at center-of-mass energy $\sqrt{s}=13$ \TeV collected by the ATLAS detector \cite{atlas_exp_2008} during Run 2 of the 
...
Although DM accounts for an estimated 84.4\% of the mass density in the universe \cite{Zyla:2020zbs}, no known particle in the Standard Model can explain it.
Thus, the inquiry into its nature is a central focus of the physics programme in the collider experiments such as ATLAS and CMS~\cite{adan2023darkmattersearchescms}. 
Among the proposed hypotheses, Weakly-Interacting Massive Particles (WIMPs) $\chi$ \cite{Steigman:1984ac} gain much interest for experimental detection of DM, which typically targets the associated production of a visible mediator decaying to stable particles and a large missing transverse momentum with magnitude \met.
...
The Two-Higgs-Doublet Model (2HDM) \cite{Bauer:2017ota} plus a pseudo-scalar mediator $a$ is the simplest gauge-invariant and renormalizable extension of the simplified pseudo-scalar DM model, offering a rich phenomenology and a more complete benchmark.
...
The constraints from searches targeting signatures are summarized in the result. 
...
The HL-HLC will help meet the demand for data with up to 3-fold increase in instantaneous luminosity $\mathcal{L}$, reaching $7.5\times 10^{34}$ $\mathrm{cm}^{-2}\mathrm{s^{-1}}$~\cite{Aberle:2749422}.
...
The expected pile-up $\expval{\mu}$ will increase from $\sim50$ in Run 3 to $140$ at the end of Run 4, peaking at 200 in Run 5, and with it a steep increase in event complexity as well as the necessary CPU resources for event reconstruction.
